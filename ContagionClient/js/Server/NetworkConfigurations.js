NETWORK_CONFIGS = [];
NETWORK_CONFIGS.Xscale = 1200; //Input topologies have x/y coordinates between 0 and 1. This scales it to a typical player's screen.
NETWORK_CONFIGS.Yscale = 700;
NETWORK_CONFIGS.Xoffset = -100;
NETWORK_CONFIGS.Yoffset = -25;
...
processConfigs = async(rawPeeps, rawConnections) =>{
	...
	peepData.forEach(function (peep, index){
		if(peep[2] == 1){
			infectedPeepsString = infectedPeepsString + index + "_";
		}
	});
	infectedPeepsString = infectedPeepsString.slice(0, -1); //removes trailing comma
	var instanceID = uuidv4(); //if the server is restarted, and the nodes are reset with it, we need to keep track of what the states were at what time.
	var query = `INSERT INTO topology_initial_states VALUES (${NETWORK_CONFIGS.length}, '${instanceID}', '${infectedPeepsString}');`;
