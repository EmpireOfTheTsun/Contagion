function Peep(config){
	...
	self.neutral = true; //TODO: if neutral mode
	self.id = config.id;
	self.sim = config.sim;
	...
	self.draw = function(ctx){
		...
		var enemyColor = PEEP_COLORS[enemyFrame];
		var neutralColor = PEEP_COLORS[neutralFrame];

		var myFrame;
		if (self.neutral){
			myFrame = neutralFrame;
		}
		else{
			myFrame = self.infected ? infectedFrame : enemyFrame;
		}


		var myColor = PEEP_COLORS[4];
		// CONCLUSION SPLASH
		if(self.sim.options.CONCLUSION){
		...
		self.playerOrbits.forEach(function(orbiter){
			...
			ctx.arc(orbiter.x, orbiter.y, 12, 0, 10);
		...
		if(self.sim.config.x==0){ //HACK: (Updated Version) Can't seem to pass new variables for Sim, so I'm using the existing x value, which is always 0 for gameplay, and something else for the intro.
